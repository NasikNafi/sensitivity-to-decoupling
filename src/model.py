...
class BasicBlockWithAttn(nn.Module):
    """
    Residual Network Block
    """
    def __init__(self, n_channels, stride=1):
        super(BasicBlockWithAttn, self).__init__()

        self.conv1 = Conv2d_tf(n_channels, n_channels, kernel_size=3, stride=1, padding=(1,1))
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = Conv2d_tf(n_channels, n_channels, kernel_size=3, stride=1, padding=(1,1))
        self.stride = stride
        self.se_attn = AttnBlock(n_channels)
...
class ResNetBaseWithAttnEarly(NNBase):
    """
    Residual Network
    """
    def __init__(self, num_inputs, hidden_size=256, channels=[16,32,32]):
        super(ResNetBaseWithAttnEarly, self).__init__(hidden_size)

        self.layer1 = self._make_layer(num_inputs, channels[0])
        self.layer2v = self._make_layer_with_attn(channels[0], channels[1])
        self.layer2p = self._make_layer_with_attn(channels[0], channels[1])
        self.layer3v = self._make_layer_with_attn(channels[1], channels[2])
        self.layer3p = self._make_layer_with_attn(channels[1], channels[2])
    ...
    def _make_layer_with_attn(self, in_channels, out_channels, stride=1):
        layers = []
    ...
    def forward(self, inputs):
        ...
        xv = self.relu(self.flatten(xv))
        xv = self.relu(self.fcv(xv))

        xp = self.relu(self.flatten(xp))
        xp = self.relu(self.fcp(xp))

        return self.critic_linear(xv), xp



class ResNetBaseWithAttnLate(NNBase):
    """
    Residual Network
    """
    def __init__(self, num_inputs, hidden_size=256, channels=[16,32,32]):
        super(ResNetBaseWithAttnLate, self).__init__(hidden_size)
...
class APDACnet(nn.Module):
    """
    APDAC netowrk
    """
    def __init__(self, obs_shape, num_actions, separation, base_kwargs=None):
        super(APDACnet, self).__init__()

        if base_kwargs is None:
            base_kwargs = {}

        if separation == 'early':
            base = ResNetBaseWithAttnEarly
        elif separation == 'late':
            base = ResNetBaseWithAttnLate
